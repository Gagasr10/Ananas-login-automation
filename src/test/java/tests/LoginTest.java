...
public class LoginTest extends BaseTest {
    ...
    public Object[][] validLoginData() {
        return new Object[][]{
            {config.getProperty("valid.email"), config.getProperty("valid.password"), "John"},
    ...
    public Object[][] invalidLoginCombos() {
        return new Object[][]{
            {config.getProperty("valid.email"), "wrongPass", "Email ili lozinka nisu u redu."},
    ...
    public Object[][] passwordLengthBoundary() {
        return new Object[][]{
            {"a", "Minimum 8 karaktera."},
            {"abcdefg", "Minimum 8 karaktera."}
        };
    }
    ...
    public void testValidLogin(String email, String password, String expectedUserName) {
        ...
        // After successful login, URL should not remain the login page
        Assert.assertTrue(driver.getCurrentUrl().contains("ananas.rs") && !driver.getCurrentUrl().contains("/login"),
                "URL after successful login should be the homepage");
    ...
    public void testMaxPasswordLength() {
        ...
        log.warn("Max password length test expects 'Password too long.', but app returns generic message – this is a known bug");
    }
    ...
    public void testAccountLockoutAfterFailedAttempts() {
        ...
        log.warn("Account lockout test expects lock message, but app returns generic error – this is a known security bug");
    }
    ...
    public void testResponsiveDesign(int width, int height) {
        ...
        loginPage.login(email, password);
        Assert.assertTrue(driver.getCurrentUrl().contains("ananas.rs") && !driver.getCurrentUrl().contains("/login"),
                "URL after successful login on responsive design should be the homepage");
